...
// do issues a JSON request with retries. body, when non-nil, is JSON-encoded.
func (t *transport) do(ctx context.Context, method, path string, body any) (*response, error) {
	...
	return t.send(ctx, method, path, encoded, "application/json")
}

// doRaw issues a request with a pre-encoded body (zip bytes or a multipart
// payload for ingest) sent verbatim with the given content type.
func (t *transport) doRaw(ctx context.Context, method, path string, body []byte, contentType string) (*response, error) {
	if contentType == "" {
		contentType = "application/octet-stream"
	}
	return t.send(ctx, method, path, body, contentType)
}

func (t *transport) send(ctx context.Context, method, path string, encoded []byte, contentType string) (*response, error) {
	var lastErr error
...
func parseMetering(h http.Header) *Metering {
	...
	if v := h.Get("x-infolang-overage"); v != "" {
		if f, err := strconv.ParseFloat(v, 64); err == nil {
			m.Overage = &f
		}
	}
