...
// TestLiveProbe exercises the SDK against a real gateway. It is skipped unless
...
//
// INFOLANG_BASE_URL / INFOLANG_NAMESPACE / INFOLANG_WORKSPACE are honored if
// set; a single-grant credential resolves its workspace automatically.
func TestLiveProbe(t *testing.T) {
	...
	t.Logf("live health: healthz=%q readyz=%q", health.Status, ready.Status)

	who, err := c.Whoami(ctx)
	if err != nil {
		t.Fatalf("Whoami: %v", err)
	}
	t.Logf("live whoami: lane=%q grants=%d", who.Lane, len(who.Workspaces))
