...
func TestParseMetering(t *testing.T) {
	...
	if m.Overage == nil || *m.Overage != 2.5 {
		t.Errorf("overage: %v", m.Overage)
	}
...
func TestParseMeteringIgnoresGarbage(t *testing.T) {
	...
	h.Set("x-infolang-overage", "nah")
	m := parseMetering(h)
	if m.TokensSaved != nil || m.RepoCoverage != nil || m.Overage != nil {
		t.Errorf("garbage should be ignored: %+v", m)
	}
}

// The X-InfoLang-Overage header surfaces on typed results via Metering.
func TestOverageSurfacedOnRecall(t *testing.T) {
	ms := newMockServer(t, func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("X-InfoLang-Overage", "3")
	...
	if res.Metering == nil || res.Metering.Overage == nil || *res.Metering.Overage != 3 {
		t.Errorf("overage not surfaced: %+v", res.Metering)
	}
}
